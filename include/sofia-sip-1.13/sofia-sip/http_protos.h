...
/* Declare internal prototypes for Sec-Websocket-Key header */

/**@addtogroup http_sec_websocket_key*/ /** @{ */

enum {
  /** Hash of Sec-Websocket-Key header. @internal*/
  http_sec_websocket_key_hash = 51441
};

/**Header class for HTTP Sec-Websocket-Key header.
 *
 * The header class http_sec_websocket_key_class defines how a HTTP
 * Sec-Websocket-Key header is parsed and printed.  It also
 * contains methods used by HTTP parser and other functions
 * to manipulate the http_sec_websocket_key_t header structure.
 *
 */
SOFIAPUBVAR msg_hclass_t http_sec_websocket_key_class[];

#ifndef HTTP_HCLASSES_ONLY

/** Decode (parse) a Sec-Websocket-Key header. @internal */
SOFIAPUBFUN msg_parse_f http_sec_websocket_key_d;

/** Encode (print) a Sec-Websocket-Key header. @internal */
SOFIAPUBFUN msg_print_f http_sec_websocket_key_e;

/**Initializer for structure http_sec_websocket_key_t.
 *
 * A static http_sec_websocket_key_t structure must be initialized
 * with the HTTP_SEC_WEBSOCKET_KEY_INIT() macro. For instance,
 * @code
 *
 *  http_sec_websocket_key_t http_sec_websocket_key = HTTP_SEC_WEBSOCKET_KEY_INIT;
 *
 * @endcode
 * @HI
 */
#define HTTP_SEC_WEBSOCKET_KEY_INIT() HTTP_HDR_INIT(sec_websocket_key)

/**Initialize a structure http_sec_websocket_key_t.
 *
 * An http_sec_websocket_key_t structure can be initialized with the
 * http_sec_websocket_key_init() function/macro. For instance,
 * @code
 *
 *  http_sec_websocket_key_t http_sec_websocket_key;
 *
 *  http_sec_websocket_key_init(&http_sec_websocket_key);
 *
 * @endcode
 * @HI
 */
#if SU_HAVE_INLINE
su_inline http_sec_websocket_key_t *http_sec_websocket_key_init(http_sec_websocket_key_t x[1])
{
  return HTTP_HEADER_INIT(x, http_sec_websocket_key_class, sizeof(http_sec_websocket_key_t));
}
#else
#define http_sec_websocket_key_init(x) \
  HTTP_HEADER_INIT(x, http_sec_websocket_key_class, sizeof(http_sec_websocket_key_t))
#endif

/**Test if header object is instance of http_sec_websocket_key_t.
 *
 * The function http_is_sec_websocket_key() returns true (nonzero) if
 * the header class is an instance of Sec-Websocket-Key header
 * object and false (zero) otherwise.
 *
 * @param header pointer to the header structure to be tested
 *
 * @return
 * The function http_is_xsec_websocket_key() returns true (nonzero) if
 * the header object is an instance of header sec_websocket_key and
 * false (zero) otherwise.
 */
#if SU_HAVE_INLINE
su_inline
...
{
  return header && header->sh_class->hc_hash == http_sec_websocket_key_hash;
}
#else
#define http_is_sec_websocket_key(h) \
 ((h) && ((msg_common_t *)(h))->h_class->hc_hash == http_sec_websocket_key_hash)
#endif

/**Duplicate (deep copy) @c http_sec_websocket_key_t.
 *
 * The function http_sec_websocket_key_dup() duplicates a header
 * structure @a hdr.  If the header structure @a hdr
 * contains a reference (@c hdr->x_next) to a list of
 * headers, all the headers in the list are duplicated, too.
 *
 ...
 * @code
 *
 *   sec_websocket_key = http_sec_websocket_key_dup(home, http->http_sec_websocket_key);
 *
 * @endcode
 *
 * @return
 * The function http_sec_websocket_key_dup() returns a pointer to the
 * newly duplicated http_sec_websocket_key_t header structure, or NULL
 * upon an error.
 */
#if SU_HAVE_INLINE
su_inline
#endif
http_sec_websocket_key_t *http_sec_websocket_key_dup(su_home_t *home,
				   http_sec_websocket_key_t const *hdr)
     __attribute__((__malloc__));

/**Copy a http_sec_websocket_key_t header structure.
 *
 * The function http_sec_websocket_key_copy() copies a header structure @a
 * hdr.  If the header structure @a hdr contains a reference (@c
 * hdr->h_next) to a list of headers, all the headers in that
 * list are copied, too. The function uses given memory @a home
 * to allocate all the memory areas used to copy the header
 ...
 * @code
 *
 *   sec_websocket_key = http_sec_websocket_key_copy(home, http->http_sec_websocket_key);
 *
 * @endcode
 *
 * @return
 * The function http_sec_websocket_key_copy() returns a pointer to
 * newly copied header structure, or NULL upon an error.
 */
#if SU_HAVE_INLINE
su_inline
#endif
http_sec_websocket_key_t *http_sec_websocket_key_copy(su_home_t *home,
				    http_sec_websocket_key_t const *hdr)
     __attribute__((__malloc__));

/**Make a header structure http_sec_websocket_key_t.
 *
 * The function http_sec_websocket_key_make() makes a new
 * http_sec_websocket_key_t header structure.  It allocates a new
 * header structure, and decodes the string @a s as the
 * value of the structure.
 *
 * @param home memory home used to allocate new header structure.
 ...
 *
 * @return
 * The function http_sec_websocket_key_make() returns a pointer to
 * newly maked http_sec_websocket_key_t header structure, or NULL upon
 * an error.
 */
#if SU_HAVE_INLINE
su_inline
...
/**Make a Sec-Websocket-Key header from formatting result.
 *
 * The function http_sec_websocket_key_format() makes a new
 * Sec-Websocket-Key header object using formatting result as its
 * value.  The function first prints the arguments according to
 * the format @a fmt specified.  Then it allocates a new header
 * structure, and uses the formatting result as the header
 * value.
 ...
 *
 * @return
 * The function http_sec_websocket_key_format() returns a pointer to newly
 * makes header structure, or NULL upon an error.
 *
 * @HIDE
 */
...
http_sec_websocket_key_t *http_sec_websocket_key_format(su_home_t *home, char const *fmt, ...)
{
  ...
  va_start(ap, fmt);
  h = http_header_vformat(home, http_sec_websocket_key_class, fmt, ap);
  va_end(ap);

  return (http_sec_websocket_key_t *)h;
}

su_inline
http_sec_websocket_key_t *http_sec_websocket_key_dup(su_home_t *home, http_sec_websocket_key_t const *o)
{
  return (http_sec_websocket_key_t *)
    msg_header_dup_as(home, http_sec_websocket_key_class, (msg_header_t const *)o);
}

su_inline
http_sec_websocket_key_t *http_sec_websocket_key_copy(su_home_t *home, http_sec_websocket_key_t const *o)
{
  return (http_sec_websocket_key_t *)
    msg_header_copy_as(home, http_sec_websocket_key_class, (msg_header_t const *)o);
}

su_inline
http_sec_websocket_key_t *http_sec_websocket_key_make(su_home_t *home, char const *s)
{
  return (http_sec_websocket_key_t *)http_header_make(home, http_sec_websocket_key_class, s);
}
#endif
...
/* Declare internal prototypes for Origin header */

/**@addtogroup http_origin*/ /** @{ */

enum {
  /** Hash of Origin header. @internal*/
  http_origin_hash = 57708
};

/**Header class for HTTP Origin header.
 *
 * The header class http_origin_class defines how a HTTP
 * Origin header is parsed and printed.  It also
 * contains methods used by HTTP parser and other functions
 * to manipulate the http_origin_t header structure.
 *
 */
SOFIAPUBVAR msg_hclass_t http_origin_class[];

#ifndef HTTP_HCLASSES_ONLY

/** Decode (parse) a Origin header. @internal */
SOFIAPUBFUN msg_parse_f http_origin_d;

/** Encode (print) a Origin header. @internal */
SOFIAPUBFUN msg_print_f http_origin_e;

/**Initializer for structure http_origin_t.
 *
 * A static http_origin_t structure must be initialized
 * with the HTTP_ORIGIN_INIT() macro. For instance,
 * @code
 *
 *  http_origin_t http_origin = HTTP_ORIGIN_INIT;
 *
 * @endcode
 * @HI
 */
#define HTTP_ORIGIN_INIT() HTTP_HDR_INIT(origin)

/**Initialize a structure http_origin_t.
 *
 * An http_origin_t structure can be initialized with the
 * http_origin_init() function/macro. For instance,
 * @code
 *
 *  http_origin_t http_origin;
 *
 *  http_origin_init(&http_origin);
 *
 * @endcode
 * @HI
 */
#if SU_HAVE_INLINE
su_inline http_origin_t *http_origin_init(http_origin_t x[1])
{
  return HTTP_HEADER_INIT(x, http_origin_class, sizeof(http_origin_t));
}
#else
#define http_origin_init(x) \
  HTTP_HEADER_INIT(x, http_origin_class, sizeof(http_origin_t))
#endif

/**Test if header object is instance of http_origin_t.
 *
 * The function http_is_origin() returns true (nonzero) if
 * the header class is an instance of Origin header
 * object and false (zero) otherwise.
 *
 * @param header pointer to the header structure to be tested
 *
 * @return
 * The function http_is_xorigin() returns true (nonzero) if
 * the header object is an instance of header origin and
 * false (zero) otherwise.
 */
#if SU_HAVE_INLINE
su_inline
int http_is_origin(http_header_t const *header)
{
  return header && header->sh_class->hc_hash == http_origin_hash;
}
#else
#define http_is_origin(h) \
 ((h) && ((msg_common_t *)(h))->h_class->hc_hash == http_origin_hash)
#endif

/**Duplicate (deep copy) @c http_origin_t.
 *
 * The function http_origin_dup() duplicates a header
 * structure @a hdr.  If the header structure @a hdr
 * contains a reference (@c hdr->x_next) to a list of
 * headers, all the headers in the list are duplicated, too.
 *
 ...
 * @code
 *
 *   origin = http_origin_dup(home, http->http_origin);
 *
 * @endcode
 *
 * @return
 * The function http_origin_dup() returns a pointer to the
 * newly duplicated http_origin_t header structure, or NULL
 * upon an error.
 */
#if SU_HAVE_INLINE
su_inline
#endif
http_origin_t *http_origin_dup(su_home_t *home,
				   http_origin_t const *hdr)
     __attribute__((__malloc__));

/**Copy a http_origin_t header structure.
 *
 * The function http_origin_copy() copies a header structure @a
 * hdr.  If the header structure @a hdr contains a reference (@c
 * hdr->h_next) to a list of headers, all the headers in that
 * list are copied, too. The function uses given memory @a home
 * to allocate all the memory areas used to copy the header
 ...
 * @code
 *
 *   origin = http_origin_copy(home, http->http_origin);
 *
 * @endcode
 *
 * @return
 * The function http_origin_copy() returns a pointer to
 * newly copied header structure, or NULL upon an error.
 */
#if SU_HAVE_INLINE
su_inline
#endif
http_origin_t *http_origin_copy(su_home_t *home,
				    http_origin_t const *hdr)
     __attribute__((__malloc__));

/**Make a header structure http_origin_t.
 *
 * The function http_origin_make() makes a new
 * http_origin_t header structure.  It allocates a new
 * header structure, and decodes the string @a s as the
 * value of the structure.
 *
 * @param home memory home used to allocate new header structure.
 ...
 *
 * @return
 * The function http_origin_make() returns a pointer to
 * newly maked http_origin_t header structure, or NULL upon
 * an error.
 */
#if SU_HAVE_INLINE
su_inline
#endif
http_origin_t *http_origin_make(su_home_t *home, char const *s)
     __attribute__((__malloc__));

/**Make a Origin header from formatting result.
 *
 * The function http_origin_format() makes a new
 * Origin header object using formatting result as its
 * value.  The function first prints the arguments according to
 * the format @a fmt specified.  Then it allocates a new header
 * structure, and uses the formatting result as the header
 * value.
 ...
 *
 * @return
 * The function http_origin_format() returns a pointer to newly
 * makes header structure, or NULL upon an error.
 *
 * @HIDE
 */
...
su_inline
#endif
http_origin_t *http_origin_format(su_home_t *home, char const *fmt, ...)
     __attribute__((__malloc__, __format__ (printf, 2, 3)));
...
http_origin_t *http_origin_format(su_home_t *home, char const *fmt, ...)
{
  ...
  va_start(ap, fmt);
  h = http_header_vformat(home, http_origin_class, fmt, ap);
  va_end(ap);

  return (http_origin_t *)h;
}

su_inline
http_origin_t *http_origin_dup(su_home_t *home, http_origin_t const *o)
{
  return (http_origin_t *)
    msg_header_dup_as(home, http_origin_class, (msg_header_t const *)o);
}

su_inline
http_origin_t *http_origin_copy(su_home_t *home, http_origin_t const *o)
{
  return (http_origin_t *)
    msg_header_copy_as(home, http_origin_class, (msg_header_t const *)o);
}

su_inline
http_origin_t *http_origin_make(su_home_t *home, char const *s)
{
  return (http_origin_t *)http_header_make(home, http_origin_class, s);
}
#endif
...
/* Declare internal prototypes for Sec-Websocket-Protocol header */

/**@addtogroup http_sec_websocket_protocol*/ /** @{ */

enum {
  /** Hash of Sec-Websocket-Protocol header. @internal*/
  http_sec_websocket_protocol_hash = 50470
};

/**Header class for HTTP Sec-Websocket-Protocol header.
 *
 * The header class http_sec_websocket_protocol_class defines how a HTTP
 * Sec-Websocket-Protocol header is parsed and printed.  It also
 * contains methods used by HTTP parser and other functions
 * to manipulate the http_sec_websocket_protocol_t header structure.
 *
 */
SOFIAPUBVAR msg_hclass_t http_sec_websocket_protocol_class[];

#ifndef HTTP_HCLASSES_ONLY

/** Decode (parse) a Sec-Websocket-Protocol header. @internal */
SOFIAPUBFUN msg_parse_f http_sec_websocket_protocol_d;

/** Encode (print) a Sec-Websocket-Protocol header. @internal */
SOFIAPUBFUN msg_print_f http_sec_websocket_protocol_e;

/**Initializer for structure http_sec_websocket_protocol_t.
 *
 * A static http_sec_websocket_protocol_t structure must be initialized
 * with the HTTP_SEC_WEBSOCKET_PROTOCOL_INIT() macro. For instance,
 * @code
 *
 *  http_sec_websocket_protocol_t http_sec_websocket_protocol = HTTP_SEC_WEBSOCKET_PROTOCOL_INIT;
 *
 * @endcode
 * @HI
 */
#define HTTP_SEC_WEBSOCKET_PROTOCOL_INIT() HTTP_HDR_INIT(sec_websocket_protocol)

/**Initialize a structure http_sec_websocket_protocol_t.
 *
 * An http_sec_websocket_protocol_t structure can be initialized with the
 * http_sec_websocket_protocol_init() function/macro. For instance,
 * @code
 *
 *  http_sec_websocket_protocol_t http_sec_websocket_protocol;
 *
 *  http_sec_websocket_protocol_init(&http_sec_websocket_protocol);
 *
 * @endcode
 * @HI
 */
#if SU_HAVE_INLINE
su_inline http_sec_websocket_protocol_t *http_sec_websocket_protocol_init(http_sec_websocket_protocol_t x[1])
{
  return HTTP_HEADER_INIT(x, http_sec_websocket_protocol_class, sizeof(http_sec_websocket_protocol_t));
}
#else
#define http_sec_websocket_protocol_init(x) \
  HTTP_HEADER_INIT(x, http_sec_websocket_protocol_class, sizeof(http_sec_websocket_protocol_t))
#endif

/**Test if header object is instance of http_sec_websocket_protocol_t.
 *
 * The function http_is_sec_websocket_protocol() returns true (nonzero) if
 * the header class is an instance of Sec-Websocket-Protocol header
 * object and false (zero) otherwise.
 *
 * @param header pointer to the header structure to be tested
 *
 * @return
 * The function http_is_xsec_websocket_protocol() returns true (nonzero) if
 * the header object is an instance of header sec_websocket_protocol and
 * false (zero) otherwise.
 */
#if SU_HAVE_INLINE
su_inline
int http_is_sec_websocket_protocol(http_header_t const *header)
{
  return header && header->sh_class->hc_hash == http_sec_websocket_protocol_hash;
}
#else
#define http_is_sec_websocket_protocol(h) \
 ((h) && ((msg_common_t *)(h))->h_class->hc_hash == http_sec_websocket_protocol_hash)
#endif

/**Duplicate (deep copy) @c http_sec_websocket_protocol_t.
 *
 * The function http_sec_websocket_protocol_dup() duplicates a header
 * structure @a hdr.  If the header structure @a hdr
 * contains a reference (@c hdr->x_next) to a list of
 * headers, all the headers in the list are duplicated, too.
 *
 ...
 * @code
 *
 *   sec_websocket_protocol = http_sec_websocket_protocol_dup(home, http->http_sec_websocket_protocol);
 *
 * @endcode
 *
 * @return
 * The function http_sec_websocket_protocol_dup() returns a pointer to the
 * newly duplicated http_sec_websocket_protocol_t header structure, or NULL
 * upon an error.
 */
#if SU_HAVE_INLINE
su_inline
#endif
http_sec_websocket_protocol_t *http_sec_websocket_protocol_dup(su_home_t *home,
				   http_sec_websocket_protocol_t const *hdr)
     __attribute__((__malloc__));

/**Copy a http_sec_websocket_protocol_t header structure.
 *
 * The function http_sec_websocket_protocol_copy() copies a header structure @a
 * hdr.  If the header structure @a hdr contains a reference (@c
 * hdr->h_next) to a list of headers, all the headers in that
 * list are copied, too. The function uses given memory @a home
 * to allocate all the memory areas used to copy the header
 ...
 * @code
 *
 *   sec_websocket_protocol = http_sec_websocket_protocol_copy(home, http->http_sec_websocket_protocol);
 *
 * @endcode
 *
 * @return
 * The function http_sec_websocket_protocol_copy() returns a pointer to
 * newly copied header structure, or NULL upon an error.
 */
#if SU_HAVE_INLINE
su_inline
#endif
http_sec_websocket_protocol_t *http_sec_websocket_protocol_copy(su_home_t *home,
				    http_sec_websocket_protocol_t const *hdr)
     __attribute__((__malloc__));

/**Make a header structure http_sec_websocket_protocol_t.
 *
 * The function http_sec_websocket_protocol_make() makes a new
 * http_sec_websocket_protocol_t header structure.  It allocates a new
 * header structure, and decodes the string @a s as the
 * value of the structure.
 *
 * @param home memory home used to allocate new header structure.
 ...
 *
 * @return
 * The function http_sec_websocket_protocol_make() returns a pointer to
 * newly maked http_sec_websocket_protocol_t header structure, or NULL upon
 * an error.
 */
#if SU_HAVE_INLINE
su_inline
#endif
http_sec_websocket_protocol_t *http_sec_websocket_protocol_make(su_home_t *home, char const *s)
     __attribute__((__malloc__));

/**Make a Sec-Websocket-Protocol header from formatting result.
 *
 * The function http_sec_websocket_protocol_format() makes a new
 * Sec-Websocket-Protocol header object using formatting result as its
 * value.  The function first prints the arguments according to
 * the format @a fmt specified.  Then it allocates a new header
 * structure, and uses the formatting result as the header
 * value.
 ...
 *
 * @return
 * The function http_sec_websocket_protocol_format() returns a pointer to newly
 * makes header structure, or NULL upon an error.
 *
 * @HIDE
 */
...
su_inline
#endif
http_sec_websocket_protocol_t *http_sec_websocket_protocol_format(su_home_t *home, char const *fmt, ...)
     __attribute__((__malloc__, __format__ (printf, 2, 3)));
...
http_sec_websocket_protocol_t *http_sec_websocket_protocol_format(su_home_t *home, char const *fmt, ...)
{
  ...
  va_start(ap, fmt);
  h = http_header_vformat(home, http_sec_websocket_protocol_class, fmt, ap);
  va_end(ap);

  return (http_sec_websocket_protocol_t *)h;
}

su_inline
http_sec_websocket_protocol_t *http_sec_websocket_protocol_dup(su_home_t *home, http_sec_websocket_protocol_t const *o)
{
  return (http_sec_websocket_protocol_t *)
    msg_header_dup_as(home, http_sec_websocket_protocol_class, (msg_header_t const *)o);
}

su_inline
http_sec_websocket_protocol_t *http_sec_websocket_protocol_copy(su_home_t *home, http_sec_websocket_protocol_t const *o)
{
  return (http_sec_websocket_protocol_t *)
    msg_header_copy_as(home, http_sec_websocket_protocol_class, (msg_header_t const *)o);
}

su_inline
http_sec_websocket_protocol_t *http_sec_websocket_protocol_make(su_home_t *home, char const *s)
{
  return (http_sec_websocket_protocol_t *)http_header_make(home, http_sec_websocket_protocol_class, s);
}
#endif
...
/* Declare internal prototypes for Sec-Websocket-Version header */

/**@addtogroup http_sec_websocket_version*/ /** @{ */

enum {
  /** Hash of Sec-Websocket-Version header. @internal*/
  http_sec_websocket_version_hash = 61506
};

/**Header class for HTTP Sec-Websocket-Version header.
 *
 * The header class http_sec_websocket_version_class defines how a HTTP
 * Sec-Websocket-Version header is parsed and printed.  It also
 * contains methods used by HTTP parser and other functions
 * to manipulate the http_sec_websocket_version_t header structure.
 *
 */
SOFIAPUBVAR msg_hclass_t http_sec_websocket_version_class[];

#ifndef HTTP_HCLASSES_ONLY

/** Decode (parse) a Sec-Websocket-Version header. @internal */
SOFIAPUBFUN msg_parse_f http_sec_websocket_version_d;

/** Encode (print) a Sec-Websocket-Version header. @internal */
SOFIAPUBFUN msg_print_f http_sec_websocket_version_e;

/**Initializer for structure http_sec_websocket_version_t.
 *
 * A static http_sec_websocket_version_t structure must be initialized
 * with the HTTP_SEC_WEBSOCKET_VERSION_INIT() macro. For instance,
 * @code
 *
 *  http_sec_websocket_version_t http_sec_websocket_version = HTTP_SEC_WEBSOCKET_VERSION_INIT;
 *
 * @endcode
 * @HI
 */
#define HTTP_SEC_WEBSOCKET_VERSION_INIT() HTTP_HDR_INIT(sec_websocket_version)

/**Initialize a structure http_sec_websocket_version_t.
 *
 * An http_sec_websocket_version_t structure can be initialized with the
 * http_sec_websocket_version_init() function/macro. For instance,
 * @code
 *
 *  http_sec_websocket_version_t http_sec_websocket_version;
 *
 *  http_sec_websocket_version_init(&http_sec_websocket_version);
 *
 * @endcode
 * @HI
 */
#if SU_HAVE_INLINE
su_inline http_sec_websocket_version_t *http_sec_websocket_version_init(http_sec_websocket_version_t x[1])
{
  return HTTP_HEADER_INIT(x, http_sec_websocket_version_class, sizeof(http_sec_websocket_version_t));
}
#else
#define http_sec_websocket_version_init(x) \
  HTTP_HEADER_INIT(x, http_sec_websocket_version_class, sizeof(http_sec_websocket_version_t))
#endif

/**Test if header object is instance of http_sec_websocket_version_t.
 *
 * The function http_is_sec_websocket_version() returns true (nonzero) if
 * the header class is an instance of Sec-Websocket-Version header
 * object and false (zero) otherwise.
 *
 * @param header pointer to the header structure to be tested
 *
 * @return
 * The function http_is_xsec_websocket_version() returns true (nonzero) if
 * the header object is an instance of header sec_websocket_version and
 * false (zero) otherwise.
 */
#if SU_HAVE_INLINE
su_inline
int http_is_sec_websocket_version(http_header_t const *header)
{
  return header && header->sh_class->hc_hash == http_sec_websocket_version_hash;
}
#else
#define http_is_sec_websocket_version(h) \
 ((h) && ((msg_common_t *)(h))->h_class->hc_hash == http_sec_websocket_version_hash)
#endif

/**Duplicate (deep copy) @c http_sec_websocket_version_t.
 *
 * The function http_sec_websocket_version_dup() duplicates a header
 * structure @a hdr.  If the header structure @a hdr
 * contains a reference (@c hdr->x_next) to a list of
 * headers, all the headers in the list are duplicated, too.
 *
 ...
 * @code
 *
 *   sec_websocket_version = http_sec_websocket_version_dup(home, http->http_sec_websocket_version);
 *
 * @endcode
 *
 * @return
 * The function http_sec_websocket_version_dup() returns a pointer to the
 * newly duplicated http_sec_websocket_version_t header structure, or NULL
 * upon an error.
 */
#if SU_HAVE_INLINE
su_inline
#endif
http_sec_websocket_version_t *http_sec_websocket_version_dup(su_home_t *home,
				   http_sec_websocket_version_t const *hdr)
     __attribute__((__malloc__));

/**Copy a http_sec_websocket_version_t header structure.
 *
 * The function http_sec_websocket_version_copy() copies a header structure @a
 * hdr.  If the header structure @a hdr contains a reference (@c
 * hdr->h_next) to a list of headers, all the headers in that
 * list are copied, too. The function uses given memory @a home
 * to allocate all the memory areas used to copy the header
 ...
 * @code
 *
 *   sec_websocket_version = http_sec_websocket_version_copy(home, http->http_sec_websocket_version);
 *
 * @endcode
 *
 * @return
 * The function http_sec_websocket_version_copy() returns a pointer to
 * newly copied header structure, or NULL upon an error.
 */
#if SU_HAVE_INLINE
su_inline
#endif
http_sec_websocket_version_t *http_sec_websocket_version_copy(su_home_t *home,
				    http_sec_websocket_version_t const *hdr)
     __attribute__((__malloc__));

/**Make a header structure http_sec_websocket_version_t.
 *
 * The function http_sec_websocket_version_make() makes a new
 * http_sec_websocket_version_t header structure.  It allocates a new
 * header structure, and decodes the string @a s as the
 * value of the structure.
 *
 * @param home memory home used to allocate new header structure.
 ...
 *
 * @return
 * The function http_sec_websocket_version_make() returns a pointer to
 * newly maked http_sec_websocket_version_t header structure, or NULL upon
 * an error.
 */
#if SU_HAVE_INLINE
su_inline
#endif
http_sec_websocket_version_t *http_sec_websocket_version_make(su_home_t *home, char const *s)
     __attribute__((__malloc__));

/**Make a Sec-Websocket-Version header from formatting result.
 *
 * The function http_sec_websocket_version_format() makes a new
 * Sec-Websocket-Version header object using formatting result as its
 * value.  The function first prints the arguments according to
 * the format @a fmt specified.  Then it allocates a new header
 * structure, and uses the formatting result as the header
 * value.
 ...
 *
 * @return
 * The function http_sec_websocket_version_format() returns a pointer to newly
 * makes header structure, or NULL upon an error.
 *
 * @HIDE
 */
...
su_inline
#endif
http_sec_websocket_version_t *http_sec_websocket_version_format(su_home_t *home, char const *fmt, ...)
     __attribute__((__malloc__, __format__ (printf, 2, 3)));
...
http_sec_websocket_version_t *http_sec_websocket_version_format(su_home_t *home, char const *fmt, ...)
{
  ...
  va_start(ap, fmt);
  h = http_header_vformat(home, http_sec_websocket_version_class, fmt, ap);
  va_end(ap);

  return (http_sec_websocket_version_t *)h;
}

su_inline
http_sec_websocket_version_t *http_sec_websocket_version_dup(su_home_t *home, http_sec_websocket_version_t const *o)
{
  return (http_sec_websocket_version_t *)
    msg_header_dup_as(home, http_sec_websocket_version_class, (msg_header_t const *)o);
}

su_inline
http_sec_websocket_version_t *http_sec_websocket_version_copy(su_home_t *home, http_sec_websocket_version_t const *o)
{
  return (http_sec_websocket_version_t *)
    msg_header_copy_as(home, http_sec_websocket_version_class, (msg_header_t const *)o);
}

su_inline
http_sec_websocket_version_t *http_sec_websocket_version_make(su_home_t *home, char const *s)
{
  return (http_sec_websocket_version_t *)http_header_make(home, http_sec_websocket_version_class, s);
}
#endif
